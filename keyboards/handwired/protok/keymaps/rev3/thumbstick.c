...
int8_t thumbstick_get_component(pin_t pin) {
    writePinHigh(THUMBSTICK_VCC);
    int16_t value = analogReadPin(pin);  // range of [0 to 4095]
    writePinLow(THUMBSTICK_VCC);
#if defined THUMBSTICK_DEBUG
...
#endif
    return (value - THUMBSTICK_RANGE_CENTER) / (0x1u << (THUMBSTICK_RAW_RES - THUMBSTICK_VEC_RES));
}
